...
export default class RestaurantController implements Crud {
    ...
    async create(request: Request, response: Response){
        const {adress, name, typeFood} = request.body;

        const isAnyUndefined = Citi.areValuesUndefined(adress, name, typeFood);
        if(isAnyUndefined) return response.status(400).send();

        const newRestaurant = { adress, name, typeFood };
    ...
    async update(request: Request, response: Response){
        ...
        const isAnyUndefined = Citi.areValuesUndefined(adress, name, typeFood, id);
        if (isAnyUndefined) return response.status(400).send();

        const restaurantWithUpdatedValues = { adress, name, typeFood };
